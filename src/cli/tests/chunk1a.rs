...
fn cli_rejects_removed_plan_flag_refresh_releases() {
    // `--refresh-releases` was removed; clap must reject it. Pin
    // the regression so a future "let's add the flag back even
    // though it's not implemented" change fails CI.
